...
type ColumnType = ColumnDef<CompanyInterface, unknown>;
...
export function CompanyListPage(props: CompanyListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getCompanies({
        relations: ['user', 'employee.count', 'hr.count', 'job.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CompanyInterface>>(
    () => `/companies?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CompanyInterface) => {
    if (hasAccess('company', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/companies/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('employee', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.employee,
        }
      : null,
    hasAccess('hr', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'hr',
          header: 'Hr',
          accessorKey: 'hr',
          cell: ({ row: { original: record } }: any) => record?._count?.hr,
        }
      : null,
    hasAccess('job', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Job',
          accessorKey: 'job',
          cell: ({ row: { original: record } }: any) => record?._count?.job,
        }
      : null,
