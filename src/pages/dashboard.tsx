...
import { ApplicationListPage as ShowCaseList } from 'pages/applications';

function DashboardPage() {
  const { session } = useSession();

  const hrStat = useSWR<PaginatedInterface<HrInterface>>(
    () => `/hrs`,
    () => getHrs({ limit: 1, offset: 0 }),
  );

  const employeeStat = useSWR<PaginatedInterface<EmployeeInterface>>(
    () => `/employees`,
    () => getEmployees({ limit: 1, offset: 0 }),
  );

  const jobStat = useSWR<PaginatedInterface<JobInterface>>(
    () => `/jobs`,
    () => getJobs({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Hrs',
      result: hrStat?.data?.totalCount,
      isLoading: hrStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Employees',
      result: employeeStat?.data?.totalCount,
      isLoading: employeeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Jobs',
      result: jobStat?.data?.totalCount,
      isLoading: jobStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`applications`} passHref legacyBehavior>
                  <Button
