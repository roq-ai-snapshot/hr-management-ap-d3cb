...
import { getHrs, deleteHrById } from 'apiSdk/hrs';
import { HrInterface } from 'interfaces/hr';

type ColumnType = ColumnDef<HrInterface, unknown>;

interface HrListPageProps {
...
export function HrListPage(props: HrListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getHrs({
        ...
        order: params.order,
        searchTermKeys: ['position.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<HrInterface>>(
    () => `/hrs?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: HrInterface) => {
    if (hasAccess('hr', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/hrs/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/hrs/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('hr', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/hrs/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('hr', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="hr" />
        </Flex>

        {hasAccess('hr', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/hrs/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
